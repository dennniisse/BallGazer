...
%             self.autoDepthSelection();
        ...
        function ColourDetection(self)
            ...
            for object = 1:length(stats)
                ...
                plot(ballCentroid(1),ballCentroid(2), '-m+')
                a=text(ballCentroid(1)+15,ballCentroid(2), strcat('X: ', num2str(round(ballCentroid(1))), ' Y: ', num2str(round(ballCentroid(2)))));
                set(a, 'FontName', 'Arial', 'FontWeight', 'bold', 'FontSize', 12, 'Color', 'black');
            end
            self.ballCentroid(1) = round(ballCentroid(1)); %disp(self.ballCentroid(1))
            self.ballCentroid(2) = round(ballCentroid(2)); %disp(self.ballCentroid(2))
            hold off;
        end
